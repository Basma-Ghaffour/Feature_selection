...
```{r}
set.seed(1)
filterCtrl2 <- sbfControl(method = "repeatedcv", repeats = 1, verbose=TRUE,functions = lmSBF,number=5)
```
...
data <- data[Filtre,]
set.seed(1)
split_data <- initial_split(data_trf, prop = 0.75, strata = G3)
data_train<- training(split_data)
data_test<- testing(split_data)
...
mtry_grid <- data.frame(mtry = 1:71)
...
## Séléction pour régréssion linéaire

```{r}
names<-colnames(data_trf)
var_lm<-NULL
...
```{r}
set.seed(1)
split_data <- initial_split(data_trf[,c(var_lm,5)], prop = 0.75, strata = G3)
data_train<- training(split_data)
data_test<- testing(split_data)
data_cv<-vfold_cv(data_train)
rec<-recipe(G3~.,data=data_train)
...
```{r}
vect1<-rbind(v1,v2,v3,v4,v5,v6)
vect2<-rbind(v11,v22,v33,v44,v55,v66)
tab_filtre<-cbind(vect1,vect2)
tab_filtre<-tab_filtre %>% 
  round(3)
colnames(tab_filtre)=c("RMSE","R2","RMSE","R2")
rownames(tab_filtre)=c("Arbre de régréssion","Forêt Aléatoire","SVR linéaire","SVR non linéaire","Régréssion linéaire","KNN")
saveRDS(tab_filtre,file="C:/Users/basma/Desktop/Memoire/Dashbord/tableau_filtre_d1.RDS")
cat(kbl(tab_filtre,booktabs = TRUE,format="latex",caption = "selection avec Filtre",align = 'c') %>% 
...
```{r}
saveRDS(tab_filtre,file="C:/Users/basma/Desktop/Memoire/Dashbord/vect_sans_s_d1transf.RDS")
```
...
```{r}
set.seed(1)
initial<-lm(G3~.,data=data_train)
library(MASS)
step<-stepAIC(initial, direction = "forward")
...
step_v5_f<-lm_fit %>% collect_metrics()
step_v5_f<-t(as.matrix(step_v5_f[,3]))
```
...
```{r}
v5<-matrix(c(1.947,0.819),ncol=2)
...
saveRDS(table_step,file="C:/Users/basma/Desktop/Memoire/Dashbord/tableau_step_1.RDS")
...
# Recursive Feathure élimination
...
saveRDS(lm_rfe,file="C:/Users/basma/Desktop/Memoire/Dashbord/lmrfe.RDS")
```
...
saveRDS(table_rfe,file="C:/Users/basma/Desktop/Memoire/Dashbord/tableau_rfe_d1.RDS")
